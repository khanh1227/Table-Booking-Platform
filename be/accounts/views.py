...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        if user.role == 'CUSTOMER' and hasattr(user, 'customer'):
            ...
            # Nếu Partner cũng có Customer profile (cho hoàn tiền/đặt bàn), thêm vào luôn
...
class ProfileView(APIView):
    ...
    def get(self, request):
        ...
        if user.role == 'CUSTOMER' and hasattr(user, 'customer'):
            ...
            # Nếu Partner cũng có Customer profile, thêm vào luôn
            if hasattr(user, 'customer') and user.customer is not None:
                data['customer'] = CustomerDetailSerializer(user.customer).data
...
class UpdateLocationView(APIView):
    """Cập nhật tọa độ và địa chỉ cuối cùng của user"""
    permission_classes = [IsAuthenticated]
