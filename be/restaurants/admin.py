...
from .models import (
    ...
    TimeSlot, CuisineAlias, QueryLog, SearchImpression
)


@admin.register(QueryLog)
class QueryLogAdmin(admin.ModelAdmin):
    list_display = ('query_text', 'user', 'restaurant_id', 'clicked', 'created_at')
...
class SearchImpressionAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('result_restaurant_ids', 'created_at')


@admin.register(Location)
class LocationAdmin(admin.ModelAdmin):
    list_display = ['id', 'city', 'district', 'ward', 'get_full_address']
    search_fields = ['city', 'district', 'ward']
    list_filter = ['city']
...
class TimeSlotInline(admin.TabularInline):
    model = TimeSlot
    extra = 1
    fields = ['start_time', 'max_bookings', 'is_active']
...
class RestaurantAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Details', {
            'fields': ('description', 'opening_hours')
        }),
        ('Status', {
