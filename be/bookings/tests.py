...
class BookingAPITestCase(TestCase):
    ...
    def test_partner_complete_booking(self):
        """Test partner hoàn thành booking"""
        past_time = (timezone.localtime() - timedelta(hours=1)).time().replace(second=0, microsecond=0)
        current_slot = TimeSlot.objects.create(
            restaurant=self.restaurant,
        ...
        response = self.client.put(f"/api/bookings/{booking.id}/complete/")
