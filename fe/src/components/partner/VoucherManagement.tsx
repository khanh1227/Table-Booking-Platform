    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[9999] animate-fadeIn">
          <div className="bg-white rounded-3xl p-8 w-full max-w-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] max-h-[90vh] overflow-y-auto custom-scrollbar relative text-slate-900">
            ...
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-bold text-slate-900 mb-1.5">Mã voucher *</label>
                <input value={formCode} onChange={e => setFormCode(e.target.value.toUpperCase())}
                  placeholder="VD: GIAM20K" className="w-full px-4 py-3.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all shadow-sm text-slate-900" />
              </div>

              <div>
                <label className="block text-sm font-bold text-slate-900 mb-1.5">Mô tả</label>
                <input value={formDesc} onChange={e => setFormDesc(e.target.value)}
                  placeholder="VD: Giảm 20k cho đơn từ 200k" className="w-full px-4 py-3.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all shadow-sm text-slate-900" />
              </div>

              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-bold text-slate-900 mb-1.5">Loại giảm *</label>
                  <select value={formType} onChange={e => setFormType(e.target.value as any)}
                    className="w-full px-4 py-3.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all shadow-sm bg-white text-slate-900">
                    <option value="PERCENTAGE">Giảm %</option>
                    <option value="FIXED">Giảm cố định (VNĐ)</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-900 mb-1.5">Giá trị giảm *</label>
                  <input type="number" value={formValue} onChange={e => setFormValue(e.target.value)}
                    placeholder={formType === 'PERCENTAGE' ? '10' : '20000'}
                    className="w-full px-4 py-3.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all shadow-sm text-slate-900" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-bold text-slate-900 mb-1.5">Giảm tối đa (VNĐ)</label>
                  <input type="number" value={formMaxDiscount} onChange={e => setFormMaxDiscount(e.target.value)}
                    placeholder="50000" className="w-full px-4 py-3.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all shadow-sm text-slate-900" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-900 mb-1.5">Đơn tối thiểu (VNĐ)</label>
                  <input type="number" value={formMinOrder} onChange={e => setFormMinOrder(e.target.value)}
                    placeholder="0" className="w-full px-4 py-3.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all shadow-sm text-slate-900" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-bold text-slate-900 mb-1.5">Bắt đầu *</label>
                  <input type="datetime-local" value={formValidFrom} onChange={e => setFormValidFrom(e.target.value)}
                    className="w-full px-4 py-3.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all shadow-sm text-slate-900" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-slate-900 mb-1.5">Kết thúc *</label>
              ...
              <div>
                <label className="block text-sm font-bold text-slate-900 mb-1.5">Giới hạn sử dụng</label>
                <input type="number" value={formUsageLimit} onChange={e => setFormUsageLimit(e.target.value)}
                  placeholder="100" className="w-full px-4 py-3.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all shadow-sm text-slate-900" />
              </div>
            </div>

            <div className="flex gap-4 mt-10">
              <button onClick={() => setShowModal(false)}
                className="flex-1 py-4 border border-slate-200 rounded-2xl font-bold text-slate-600 hover:bg-slate-50 transition-all hover:border-slate-300">
                Hủy
              </button>
              <button onClick={handleSubmit} disabled={submitting}
                className="flex-1 py-4 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 disabled:opacity-50 text-white rounded-2xl font-bold transition-all shadow-lg shadow-amber-200 flex items-center justify-center">
                {submitting ? 'Đang xử lý...' : editingVoucher ? 'Cập nhật Voucher' : 'Tạo Voucher Ngay'}
              </button>
            </div>
          </div>
        </div>
