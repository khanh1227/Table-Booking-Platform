...
import { Plus, Edit2, Trash2, AlertCircle, Building2, ArrowRight, Image } from 'lucide-react';
import { fetchMyRestaurants, deleteRestaurant } from '@/lib/api';
import RestaurantForm from './RestaurantForm';

interface RestaurantListProps {
  onSelectRestaurant: (restaurant: any) => void;
  onManageImages?: (restaurantId: string, restaurantName: string) => void;
}

export default function RestaurantList({ onSelectRestaurant, onManageImages }: RestaurantListProps) {
              ...
              <div className="p-8 flex-1">
                ...
                <div className="flex flex-wrap gap-4 pt-4 border-t border-slate-800/50 mt-auto">
                  ...
                  <div className="flex gap-3">
                    ...
                    {onManageImages && (
                      <button
                        onClick={() => onManageImages(String(restaurant.id), restaurant.name)}
                        className="p-4 bg-slate-800 hover:bg-blue-600 text-slate-300 hover:text-white rounded-2xl transition-all border border-slate-700 shadow-sm group/btn"
                        title="Quản lý ảnh"
                      >
