...
export default function RestaurantForm({
  ...
  const [depositPolicy, setDepositPolicy] = useState<{
    id?: number;
    is_required: boolean;
    deposit_per_guest: string;
    minimum_guests_for_deposit: number;
  }>({
    is_required: false,
    deposit_per_guest: "0",
    minimum_guests_for_deposit: 1,
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Load deposit policy
        const policy = await fetchDepositPolicy(restaurantId);
        if (policy) {
          setDepositPolicy({
            id: policy.id,
            is_required: policy.is_required,
            deposit_per_guest: policy.deposit_per_guest.toString(),
            minimum_guests_for_deposit: policy.minimum_guests_for_deposit,
          });
        }
      } catch (e: any) {
    ...
    try {
      ...
      if (!locationId && locationData.city.trim()) {
        const loc = await createLocation({
          ...
          ward_code: selectedWardCode || undefined,
        });
        locationId = loc.id;
      }
      ...
      if (restaurantId) {
        ...
        // Save deposit policy
        if (depositPolicy.id) {
          await updateDepositPolicy(depositPolicy.id, {
            is_required: depositPolicy.is_required,
            deposit_per_guest: depositPolicy.deposit_per_guest,
            minimum_guests_for_deposit: depositPolicy.minimum_guests_for_deposit,
          });
        } else if (depositPolicy.is_required || Number(depositPolicy.deposit_per_guest) > 0) {
          await createDepositPolicy({
            restaurant_id: Number(restaurantId),
            is_required: depositPolicy.is_required,
            deposit_per_guest: Number(depositPolicy.deposit_per_guest),
            minimum_guests_for_deposit: depositPolicy.minimum_guests_for_deposit,
          });
        }
      } else {
        const created = await createRestaurant(payload);
        // Save deposit policy for new restaurant
        if (depositPolicy.is_required || Number(depositPolicy.deposit_per_guest) > 0) {
          await createDepositPolicy({
            restaurant_id: created.data.id,
            is_required: depositPolicy.is_required,
            deposit_per_guest: Number(depositPolicy.deposit_per_guest),
            minimum_guests_for_deposit: depositPolicy.minimum_guests_for_deposit,
          });
      ...
      <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-8 max-w-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-slate-400 text-sm font-bold uppercase tracking-widest mb-2">
                Mức giá trung bình (VNĐ/người)
              </label>
              <div className="relative">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setFormData({ ...formData, price_range: val });
                  }}
                  placeholder="VD: 150000"
                  className="w-full px-5 py-4 bg-slate-900 border border-slate-700 rounded-2xl text-white focus:border-orange-500 outline-none transition shadow-inner"
                />
          ...
          <div className="border border-slate-700 rounded-lg p-6 bg-orange-500/5">
            <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
              <span className="w-8 h-8 bg-orange-500/20 text-orange-500 rounded-lg flex items-center justify-center text-sm">💰</span>
              Chính sách đặt cọc (Bắt buộc)
            </h3>

            <div className="space-y-6">
              <p className="text-slate-400 text-sm italic">
                * Nền tảng yêu cầu bắt buộc đặt cọc để đảm bảo uy tín của khách hàng và nhà hàng.
              </p>

              <div>
                <label className="block text-slate-400 text-sm font-bold uppercase tracking-widest mb-2">
                  Tiền cọc mỗi khách (VNĐ/người)
                </label>
                <div className="relative">
                  <input
                    type="text"
                    inputMode="numeric"
                    pattern="[0-9]*"
                    value={depositPolicy.deposit_per_guest}
                    onChange={(e) => {
                      const val = e.target.value.replace(/\D/g, "");
                      setDepositPolicy({ ...depositPolicy, deposit_per_guest: val });
                    }}
                    placeholder="Tối thiểu 10.000đ"
                    className="w-full px-5 py-4 bg-slate-900 border border-slate-700 rounded-2xl text-white focus:border-orange-500 outline-none transition"
                  />
                  <div className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-500 font-bold">
                    đ
                  </div>
                ...
                <div className="flex justify-between mt-2">
                  <p className="text-[10px] text-slate-500">Gợi ý: 50.000đ - 200.000đ tùy mức độ cao cấp</p>
                  {Number(depositPolicy.deposit_per_guest) < 10000 && depositPolicy.deposit_per_guest !== "" && (
                    <p className="text-[10px] text-red-400 font-bold">Tối thiểu 10.000đ</p>
                  )}
                </div>
              </div>
            </div>
