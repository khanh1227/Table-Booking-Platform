...
export const BookingCard: React.FC<BookingCardProps> = ({
  ...
  return (
    <div
      ...
      className={`group bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 flex flex-col md:flex-row ${isHighlighted ? 'animate-highlight ring-2 ring-orange-500' : ''
        }`}
    >
      {/* Visual Side/Top */}
      <div
      ...
      >
        ...
        <div className="absolute top-3 left-3">
          <StatusBadge status={booking.status as any} reason={booking.rejection_reason} />
        </div>
      </div>
          ...
          <div className="flex items-center gap-3 ml-auto">
            {booking.can_cancel && (
              <button
                ...
                className="px-4 py-2 text-sm font-bold text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all flex items-center gap-1.5 whitespace-nowrap"
              >
            ...
            <button
              onClick={() => onViewDetail(booking.id)}
              className="md:hidden w-10 h-10 flex items-center justify-center bg-slate-100 rounded-xl text-slate-400"
            >
