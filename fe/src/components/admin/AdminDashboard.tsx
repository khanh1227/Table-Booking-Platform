...
export default function AdminDashboard() {
  ...
  useEffect(() => { fetchData(); }, [activeRange]);

  const fetchData = async () => {
    setLoading(true);
    try {
      let url = `${API_BASE}/api/analytics/admin_dashboard/?time_range=${activeRange}`;
      if (activeRange === 'custom' && customRange.start && customRange.end) {
        url += `&start_date=${customRange.start}&end_date=${customRange.end}`;
      }
      const res = await fetch(url, { headers: { 'Authorization': `Bearer ${localStorage.getItem('access')}` } });
  ...
  return (
    <div className="space-y-6">
      {/* Time Range Selector */}
      <div className="bg-white rounded-2xl px-6 py-4 shadow-sm border border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <p className="text-sm font-bold text-slate-600 shrink-0">Xem theo kỳ:</p>
        ...
        <div className="flex items-center gap-2 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mr-2">Tùy chọn:</p>
          <div className="flex items-center gap-2">
            <input 
              type="date" 
              className="bg-slate-50 border border-slate-200 rounded-lg px-2 py-1 text-xs font-bold text-slate-600 focus:border-rose-400 outline-none"
            ...
            <button 
              onClick={() => {
                if (!customRange.start || !customRange.end) return alert('Vui lòng chọn đủ từ ngày - đến ngày');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
            ...
            <div className="flex items-center bg-slate-100 p-1 rounded-lg self-start sm:self-auto">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-bold transition-colors ${showCompletedOnly ? 'bg-white shadow-sm text-green-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
                Chỉ Hoàn thành
              </button>
            </div>
          ...
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={charts?.activity}>
                ...
                <Bar yAxisId="left" dataKey={showCompletedOnly ? "completed_bookings" : "bookings"} fill={showCompletedOnly ? "#10b981" : "#f43f5e"} radius={[4, 4, 0, 0]} name="Lượt đặt" opacity={0.85} />
          ...
          <div className="flex items-center gap-6 mt-3 justify-center">
            <div className="flex items-center gap-2 text-xs text-slate-500"><span className={`w-3 h-3 rounded-sm ${showCompletedOnly ? 'bg-emerald-500' : 'bg-rose-500'} inline-block`} />Lượt đặt</div>
            <div className="flex items-center gap-2 text-xs text-slate-500"><span className={`w-4 h-0.5 ${showCompletedOnly ? 'bg-emerald-700' : 'bg-blue-500'} inline-block`} />Lượng khách</div>
          </div>
        </div>
