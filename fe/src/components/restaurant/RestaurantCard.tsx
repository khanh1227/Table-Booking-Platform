...
import { getRestaurantImage, getPriceRangeLabel, getPriceRangeSymbol, getPriceRangeColor } from "@/lib/restaurantDisplay";
...
export default function RestaurantCard({ restaurant, onBookTable, onFavoriteChange, layout = "grid" }: RestaurantCardProps) {
  ...
  const detailedAddress = (() => {
    ...
    const parts = [loc.ward, loc.district, loc.city]
      .map(p => (p || '').trim())
      .filter(Boolean);
    ...
    const missing = parts.filter(p => !baseNorm.includes(p.toLowerCase()));
          ...
          <div>
            <div className="flex justify-between items-start mb-2">
              ...
              {restaurant.distance !== undefined && restaurant.distance !== null && (
                <div className="bg-blue-50 px-2 py-1 rounded-lg flex items-center space-x-1 shrink-0 ml-2">
            ...
            <div className="flex items-start space-x-2 text-gray-500 text-sm mb-3">
              ...
              <span className="line-clamp-2" title={detailedAddress}>{detailedAddress}</span>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {restaurant.price_range && (
                <span className={`text-xs font-semibold px-2 py-0.5 rounded ${getPriceRangeColor(restaurant.price_range)}`}>
          ...
          {onBookTable && (
            <div className="mt-4 flex justify-end">
              <button
                ...
                className="bg-red-600 text-white px-5 py-2.5 rounded-xl hover:bg-red-700 hover:shadow-lg transition-all font-bold text-sm shadow-sm"
              >
                Đặt Bàn Ngay
              </button>
            </div>
    ...
    >
      ...
      <div className="p-5 flex-1 flex flex-col">
        <Link to={`/restaurant/${restaurant.id}`} className="block mb-2">
          <h3 className="text-xl font-bold text-gray-800 line-clamp-1 group-hover:text-amber-600 transition-colors">
            {restaurant.name}
          </h3>
        </Link>

        <div className="flex items-start space-x-2 mb-3 text-gray-500 text-sm h-10">
          <MapPin className="w-4 h-4 shrink-0 mt-0.5" />
          <span className="line-clamp-2" title={detailedAddress}>{detailedAddress}</span>
        </div>
        ...
        <div className="flex items-end justify-between mt-auto pt-4 border-t border-gray-50">
          <div>
            <div className="flex items-center space-x-1 mb-1">
              ...
              {restaurant.distance !== undefined && restaurant.distance !== null && (
                <div className="flex items-center text-blue-600 ml-2 px-1.5 py-0.5 bg-blue-50 rounded-md">
            ...
            <div className="flex items-center gap-1 flex-wrap">
              ...
              {restaurant.price_range && (
                <span className={`text-[11px] font-semibold px-2 py-0.5 rounded normal-case tracking-normal ${getPriceRangeColor(restaurant.price_range)}`}>
                  {getPriceRangeLabel(restaurant.price_range)}
                </span>
              )}
            </div>
          ...
          {onBookTable ? (
            <button
              ...
              className="bg-red-600 text-white px-4 py-2 rounded-xl hover:bg-red-700 transition-all font-bold text-sm shadow-sm whitespace-nowrap shrink-0"
            >
              Đặt Bàn
            </button>
          ) : (
