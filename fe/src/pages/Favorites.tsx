  ...
  useEffect(() => {
    ...
    const fetchRecent = async () => {
      ...
      try {
        ...
        if (token) {
          // Fetch from server
          const res = await apiFetch(`${API_BASE}/api/restaurants/restaurants/view-history/?page_size=${MAX_RECENT}`, {
            headers: getAuthHeaders()
          });
          if (res.ok) {
            const data = await res.json();
            const results = data.results || data;
            const items = Array.isArray(results) ? results.map((h: any) => h.restaurant) : [];
            
            if (!cancelled) {
              setRecentRestaurants(items.filter(Boolean));
              setLoadingRecent(false);
              // Nếu server trống thì ẩn section này đi (đúng với user mới)
              if (items.length === 0) setShowRecent(false);
              return; 
            }
          } else {
            // Nếu lỗi API (vd token hết hạn), có thể hiện trống hoặc fallback. 
            // Ở đây ta chọn ẩn đi để đảm bảo tính riêng tư.
            if (!cancelled) setShowRecent(false);
            return;
          }
        }

        // Fallback to localStorage if no token or server error
        ...
        if (!cancelled) {
          const finalItems = items.filter(Boolean) as RestaurantCardDto[];
          setRecentRestaurants(finalItems);
          if (finalItems.length === 0) setShowRecent(false);
        }
      } catch {
        if (!cancelled) setShowRecent(false);
      } finally {
