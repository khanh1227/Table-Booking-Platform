...
3. Chạy ứng dụng:
   ...
   npx expo start
   ```
   **Lưu ý quan trọng để Mobile kết nối được Backend:**
   - **Backend:** Cần chạy server Django với địa chỉ IP tĩnh hoặc `0.0.0.0` để các thiết bị trong mạng nội bộ truy cập được:
     ```bash
     python manage.py runserver 0.0.0.0:8000
     ```
   - **Cấu hình Mobile:** Mở file `mobile/src/services/api.ts` và cập nhật biến `API_IP` thành địa chỉ IP máy tính của bạn (Ví dụ: `192.168.1.10`).
   - **Expo Go:** Cài ứng dụng **Expo Go** trên điện thoại và quét mã QR từ terminal để trải nghiệm.
